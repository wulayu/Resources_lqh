...
from ..layers.odconv import ODConv2d
...
class CondResBlock(nn.Module):
    def __init__(self, channel, slope=0.01, start_from_relu=True, end_with_relu=False,
        ...
        if slope < 0.0001:
            self.relu = nn.ReLU()
        if bottleneck:
            self.conv1 = ODConv2d(channel, channel // 2, 3, padding=1)
            self.conv2 = ODConv2d(channel // 2, channel, 3, padding=1)
        else:
            self.conv1 = ODConv2d(channel, channel, 3, padding=1)
            self.conv2 = ODConv2d(channel, channel, 3, padding=1)
        self.first_layer = self.relu if start_from_relu else nn.Identity()
        self.last_layer = self.relu if end_with_relu else nn.Identity()
...
def get_condition_enc_dec_models(y_channel, z_channel):
    enc = nn.Sequential(
        conv3x3(y_channel, z_channel),
        nn.LeakyReLU(),
        conv3x3(z_channel, z_channel),
...
def get_mv_hyper_enc_dec_models(in_channel, z_channel, out_channel):
    enc = nn.Sequential(
        conv3x3(in_channel, z_channel),
        nn.LeakyReLU(),
        conv3x3(z_channel, z_channel),
        nn.LeakyReLU(),
        conv3x3(z_channel, z_channel, stride=2),
    ...
    dec = nn.Sequential(
        conv3x3(z_channel, out_channel),
        nn.LeakyReLU(),
        subpel_conv1x1(out_channel, out_channel, 2),
        nn.LeakyReLU(),
        conv3x3(out_channel, out_channel * 3 // 2),
        nn.LeakyReLU(),
        subpel_conv1x1(out_channel * 3 // 2, out_channel * 3 // 2, 2),
        nn.LeakyReLU(),
        conv3x3(out_channel * 3 // 2, out_channel),
    )

    return enc, dec

def get_gaus_enc_dec_models(y_channel, z_channel):
    enc = nn.Sequential(
        conv3x3(y_channel, z_channel),
        nn.LeakyReLU(),
        conv3x3(z_channel, z_channel),
