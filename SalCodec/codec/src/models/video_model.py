...
from .video_net import ME_Spynet, flow_warp, ResBlock, bilineardownsacling, LowerBound, UNet, \
    get_enc_dec_models, get_hyper_enc_dec_models, CondResBlock
...
from ..entropy_models.entropy_models import BitEstimator
...
class ContextualEncoder(nn.Module):
    def __init__(self, channel_N=64, channel_M=96):
        ...
        self.cd_res1= CondResBlock(channel_N)
...
class CG(nn.Module):
    def __init__(self, chn) -> None:
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(chn, chn, 3, padding=1),
            nn.LeakyReLU(0.2),
            nn.Conv2d(chn, chn, 3, padding=1),
        )
        self.ctx_conv = nn.Sequential(
            nn.Conv2d(1, chn, 3, padding=1),
            nn.LeakyReLU(0.2),
            nn.Conv2d(chn, chn, 3, padding=1),
        )
        self.res1 = ResBlock(chn, bottleneck=True, slope=0.1,
            start_from_relu=True, end_with_relu=True)
        self.down1 = nn.Conv2d(chn, chn, 3, stride=2, padding=1)
        self.up1 = subpel_conv3x3(chn, chn, 2)
        self.conv2 = nn.Sequential(
            nn.Conv2d(chn, chn, 3, padding=1),
            nn.LeakyReLU(0.2),
            nn.Conv2d(chn, chn, 3, padding=1),
        )
        self.squeeze_chn = nn.Conv2d(chn, 1, 3, padding=1)
    
    def forward(self, x, ctx):
        x = self.conv1(x)
        ctx = self.ctx_conv(ctx)
        x = x * ctx
        chn = torch.nn.functional.adaptive_avg_pool2d(x, output_size=(1, 1))
        spa = self.squeeze_chn(x)
        x = self.res1(x)
        x = x + chn * spa * x
        x = self.down1(x)
        x = self.up1(x)
...
class MaskGenerator(nn.Module):
    def __init__(self, in_chn, out_chn) -> None:
        super().__init__()
        self.conv1 = nn.Conv2d(in_chn, out_chn, kernel_size=3, padding=1)
        self.cg1 = CG(out_chn)
        self.cg2 = CG(out_chn)
    ...
    def forward(self, x):
        x = self.conv1(x)
        B, C, H, W = x.shape
        ctx = torch.ones([B, 1, H, W], device=x.device)
        x, ctx = self.cg1(x, ctx)
        x, ctx = self.cg2(x, ctx)
        return x

class SalienceMap(nn.Module):
    def __init__(self, in_chn=1, out_chn=1) -> None:
        super().__init__()
        self.conv1 = nn.Conv2d(in_chn, out_chn * 2, 3, stride=2, padding=1)
        self.res1 = ResBlock(out_chn * 2)
        self.conv2 = nn.Conv2d(out_chn * 2, out_chn * 2, 3, stride=2, padding=1)
        self.res2 = ResBlock(out_chn * 2)
        self.conv3 = nn.Conv2d(out_chn * 2, out_chn * 2, 3, stride=2, padding=1)
        self.res3 = ResBlock(out_chn * 2)
        self.conv4 = nn.Conv2d(out_chn * 2, out_chn, 3, stride=2, padding=1)
        self.res4 = ResBlock(out_chn)
    def forward(self, x):
        x = self.conv1(x)
        x = self.res1(x)
        x = self.conv2(x)
...
class VideoCompressor(CompressionModel):
    def __init__(self, anchor_num=4, use_sal=False):
        ...
        self.mv_y_mask_gen = MaskGenerator(channel_mv * 3, channel_mv)
        ...
        if self.use_sal:
            self.sal_map = SalienceMap()

        self.y_mask_gen = MaskGenerator(channel_M * 3, channel_M)
    ...
    def encode_decode(self, x, dpb, output_path=None,
                      ...
                      mv_y_q_scale=None, y_q_scale=None, smap=None):
        # pic_width and pic_height may be different from x's size. x here is after padding
        # x_hat has the same size with x
        ...
        encoded = self.forward_one_frame(x, dpb,
                                         mv_y_q_scale=mv_y_q_scale, y_q_scale=y_q_scale, smap=smap)
        result = {
            "dpb": encoded['dpb'],
    ...
    def forward_one_frame(self, x, dpb, mv_y_q_scale=None, y_q_scale=None, rate_idx=0, smap=None):
        ref_frame = dpb["ref_frame"]
        if mv_y_q_scale is None or y_q_scale is None:
            mv_y_q_scale = self.mv_y_q_scale[rate_idx]
            y_q_scale = self.y_q_scale[rate_idx]
        curr_mv_y_q = self.get_curr_mv_y_q(mv_y_q_scale)
        curr_y_q = self.get_curr_y_q(y_q_scale)
        ...
        mv_params = torch.cat((mv_params, ref_mv_y), dim=1)
        mv_q_step, mv_scales, mv_means = self.mv_y_prior_fusion(mv_params).chunk(3, 1)
        mv_y_res, mv_y_q, mv_y_hat, mv_scales_hat = self.forward_prior(
            mv_y, mv_means, mv_scales, mv_q_step, self.mv_y_spatial_prior, self.mv_y_mask_gen)
        mv_y_hat = mv_y_hat * curr_mv_y_q
        ...
        params = torch.cat((temporal_params, hierarchical_params, ref_y), dim=1)
        q_step, scales, means = self.y_prior_fusion(params).chunk(3, 1)
        y_res, y_q, y_hat, scales_hat = self.forward_prior(
            y, means, scales, q_step, self.y_spatial_prior, self.y_mask_gen)
        y_hat = y_hat * curr_y_q
    ...
    def forward(self, x, dpb, mv_y_q_scale=None, y_q_scale=None, rate_idx=0, smap=None):
        return self.forward_one_frame(x, dpb,
                                      mv_y_q_scale=mv_y_q_scale, y_q_scale=y_q_scale, rate_idx=rate_idx, smap=smap)
