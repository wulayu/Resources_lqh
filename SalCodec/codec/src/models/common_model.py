...
class CompressionModel(nn.Module):
    ...
    def get_gumbel_mask(self, x, y_feature_mask_gen):
        p = y_feature_mask_gen(x).unsqueeze(-1)
        q = 1 - p
        logits = (torch.cat([p, q], dim=-1) > 0).float()
        mask = F.gumbel_softmax(logits=logits, tau=self.tau, hard=self.hard)
        mask_0, mask_1 = mask[..., 0], mask[..., 1]
        # mask_0, mask_1 = logits[..., 0], logits[..., 1]
        return mask_0, mask_1

    def forward_prior(self, y, means, scales, quant_step, y_spatial_prior, y_mask_generator, write=False):
        device = y.device
        _, C, H, W = y.size()
        mask_0_, mask_1_ = self.get_mask(H, W, device)
        mask_0, mask_1 = self.get_gumbel_mask(torch.cat([means, scales, quant_step], dim=1), y_mask_generator)
        quant_step = LowerBound.apply(quant_step, 0.5)
        y = y / quant_step
        y_0, y_1 = y.chunk(2, 1)
        ...
        y_res_0_0, y_q_0_0, y_hat_0_0, scales_hat_0_0 = \
            self.process_with_mask(y_0, scales_0, means_0, mask_0[:, :C//2])
        y_res_1_1, y_q_1_1, y_hat_1_1, scales_hat_1_1 = \
            self.process_with_mask(y_1, scales_1, means_1, mask_1[:, C//2:])

        params = torch.cat((y_hat_0_0, y_hat_1_1, means, scales, quant_step), dim=1)
        scales_0, means_0, scales_1, means_1 = y_spatial_prior(params).chunk(4, 1)

        y_res_0_1, y_q_0_1, y_hat_0_1, scales_hat_0_1 = \
            self.process_with_mask(y_0, scales_0, means_0, mask_1[:, :C//2])
        y_res_1_0, y_q_1_0, y_hat_1_0, scales_hat_1_0 = \
            self.process_with_mask(y_1, scales_1, means_1, mask_0[:, C//2:])
    ...
    def forward_progressive_prior(self, y, means, scales, quant_step, y_spatial_prior, y_mask_generator, write=False):
        pass