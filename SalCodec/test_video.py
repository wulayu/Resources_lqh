...
from codec.src.models.video_model import VideoCompressor
from codec.src.models.image_model import IntraNoAR
from codec.src.utils.common import str2bool, interpolate_log, create_folder, generate_log_json, dump_json
from codec.src.utils.stream_helper import get_padding_size, get_state_dict
from codec.src.utils.png_reader import PNGReader
from tqdm import tqdm
from pytorch_msssim import ms_ssim
import logging
import functools
import sys,traceback
from saliency.model import MobileSal
...
def parse_args():
    ...
    parser.add_argument('--i_frame_model_path', default="codec/checkpoints/image_psnr.pth.tar", type=str)
    parser.add_argument('--sal_model_path', default="output/sal-09-03_10-42-14/epoch_0007.pt", type=str)
    ...
    parser.add_argument('--model_path',  type=str, default="output/codec-10-28_21-46-29/epoch_0000_step017000.pt")
    ...
    parser.add_argument('--decoded_frame_path', type=str, default='output/decoded/dec')
    parser.add_argument('--output_path', type=str, default="salcodec.json")
...
def run_test(video_net, i_frame_net, sal_net, args, device):
    ...
    seq_name = args['img_path'].split('/')[-1]
    ...
    with torch.no_grad():
        for frame_idx in range(frame_num):
            ...
            if frame_idx % gop_size == 0:
                ...
                smap = sal_net(x_padded)
                result = video_net.encode_decode(x_padded, dpb, bin_path,
                                                 pic_height=pic_height, pic_width=pic_width, smap=smap,
            ...
            if verbose >= 2 and frame_idx % gop_size != 0:
                msg = f'{seq_name[:4]:<4s} {rate_idx} '+\
                      ...
                      f"bpp: {result['bpp']:.4f} "+\
                      f"bpp_y: {result['bpp_y']:.4f}, bpp_z: {result['bpp_z']:.4f} "+\
                      f"bpp_mv_y: {result['bpp_mv_y']:.4f}, bpp_mv_z: {result['bpp_mv_z']:.4f} "
                logger.info(msg)
            else:
                msg = f"frame {frame_idx}, {frame_end_time - frame_start_time:.3f} seconds,"+\
                      f"bits: {bits[-1]:.3f}, PSNR: {psnrs[-1]:.4f}, MS-SSIM: {msssims[-1]:.4f} "
                logger.info(msg)
    ...
    # ffmpeg: png to yuv
    cmd = ["ffmpeg", '-i', args['decoded_frame_folder']+f"/%d.png", '-pix_fmt', 'yuv420p',  "-s", 
        str(args['src_width']) + 'x' + str(args['src_height']), args['decoded_frame_folder'] + '/rec.yuv']
    ...
    # # # VQMT
    cmd = ["/data/lqh/VQMT_Saliency/build/bin/Debug/vqmt", args['yuv'], args['decoded_frame_folder'] + '/rec.yuv',
        str(args['src_width']), str(args['src_height']), frame_num, 1, 'EWPSNR', 'PSNR']
    cmd = " ".join(list(map(str, cmd)))
    logger.info(cmd)
    # os.system(cmd)
    result = sp.run(cmd, stdout=sp.PIPE, stderr=sp.STDOUT, shell=True)
    ...
    if verbose >= 1 and p_frame_number > 0:
        msg = f"decoding {p_frame_number} P frames, " +\
...
def encode_one(args, device):
    ...
    sal_state_dict = get_state_dict(args['sal_model_path'])
    sal_net = MobileSal()
    sal_net.load_state_dict(sal_state_dict)
    sal_net = sal_net.to(device)
    sal_net.eval()
    ...
    if args['force_intra']:
        ...
        video_net = VideoCompressor(use_sal=True)
    ...
    result = run_test(video_net, i_frame_net, sal_net, args, device=device)
...
def main():
    ...
    if not args.force_intra:
        p_frame_y_q_scales, p_frame_mv_y_q_scales = VideoCompressor.get_q_scales_from_ckpt(args.model_path)
    ...
    for ds_name in config:
        ...
        for seq_name in config[ds_name]['sequences']:
            ...
            for rate_idx in range(rate_num):
                cur_args = {}
                cur_args['log_path'] = os.path.join(args.log_path, "worker_{}.log".format(jobs))
                ...
                cur_args['dataset_path'] = os.path.join(root_path, ds_name, config[ds_name]['base_path'])
                ...
                cur_args['sal_model_path'] = args.sal_model_path
                cur_args['single'] = args.single
                cur_args['yuv'] = config[ds_name]['sequences'][seq_name]['yuv']
