...
from codec.src.utils.misc import get_temperature
from codec.src.dataset.dataset import MultiFrameTrainDataset
from codec.src.utils.common import dump_json
...
parser.add_argument('--saliency_model_path', default="output/sal-09-03_10-42-14/epoch_0007.pt", type=str)
parser.add_argument('-l', '--log', default='loguvg.txt',
        help='output training details')
parser.add_argument('-p', '--pretrain', default = 'checkpoints_init.pth.tar',
...
parser.add_argument('--data_path', default="/data/lqh/deepvideo/DVC/DVC/data/vimeo_septuplet", type=str)
...
if __name__ == "__main__":
    ...
    s_model = MobileSal().to(device)
    state_dict = torch.load(args.saliency_model_path, map_location='cpu')
    state_dict_ = {}
    for name, param in state_dict.items():
        name = name.replace("module.", "")
        state_dict_[name] = param
    s_model.load_state_dict(state_dict_, strict=True)
    ...
    if args.local_rank >= 0:
        ...
        i_model = DistributedDataParallel(i_model.cuda(),
                                        device_ids=[torch.cuda.current_device()],
                                        find_unused_parameters=True)
        s_model = DistributedDataParallel(s_model.cuda(),
                                        device_ids=[torch.cuda.current_device()],
                                        find_unused_parameters=True)
                ...
                for rate_idx in range(4):
                    ...
                    for frame_idx in range(imgs.shape[1]):
                        ...
                        if frame_idx == 0:
                            ...
                            continue
                        smap = s_model(img)
                        result = model(img, dpb, rate_idx=rate_idx, smap=smap)
                        dpb = result['dpb']
                        rd_loss = rd_loss + lamda[rate_idx] * frame_wieght[frame_idx] * result['mse']+ result['bpp']
